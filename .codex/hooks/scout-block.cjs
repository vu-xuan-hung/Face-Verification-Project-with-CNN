 ...
 *
 * Blocks access to directories listed in .codex/.ckignore
 ...
 *
 * Configuration:
 * - Edit .codex/.ckignore to customize blocked patterns (one per line, # for comments)
...
try {
  ...
  // Import formatters (kept local as they're Codex-specific output)
  ...
  try {
    ...
    const codexDir = path.dirname(__dirname); // Go up from hooks/ to .codex/
    ...
    const result = checkScoutBlock({
      ...
      options: {
        codexDir,
        ckignorePath: path.join(codexDir, '.ckignore'),
