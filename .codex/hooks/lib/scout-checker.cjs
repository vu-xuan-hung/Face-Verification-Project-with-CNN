 ...
 * Codex hooks and OpenCode plugins.
 *
...
// Allow execution from Python virtualenv binary folders without allowing reads of
// the virtualenv itself. The venv executable must be the command being run, not
// merely an argument to commands like `cat` or `ls`.
// Supports project venvs (.venv/bin/python) and shared user layouts such as
// ~/.venv/shared/bin/python.
const VENV_EXECUTABLE_PATTERN = /^(?:"[^"]*[\/\\]\.?venv(?:[\/\\][^\/\\\s"']+)?[\/\\](bin|Scripts)[\/\\][^"]+"|'[^']*[\/\\]\.?venv(?:[\/\\][^\/\\\s"']+)?[\/\\](bin|Scripts)[\/\\][^']+'|[^\s]*[\/\\]\.?venv(?:[\/\\][^\/\\\s"']+)?[\/\\](bin|Scripts)[\/\\][^\s]+)(\s|$)/;
 ...
 * @param {string} [params.options.codexDir] - Path to .codex or .opencode directory
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  const {
    ckignorePath,
    codexDir = path.join(process.cwd(), '.codex'),
  ...
  const resolvedCkignorePath = ckignorePath || path.join(codexDir, '.ckignore');
