 ...
 *
 * Extracted from dev-rules-reminder.cjs for reuse in both Codex hooks and OpenCode plugins.
 ...
 */
function execSafe(cmd, cwd = undefined) {
  try {
 ...
 * @param {string} [configDirName='.codex'] - Config directory name
 * @returns {string|null} Resolved path or null
 */
function resolveRulesPath(filename, configDirName = '.codex', baseDir = process.cwd()) {
  // Try rules/ first (new location)
  const localRulesPath = path.join(baseDir, configDirName, 'rules', filename);
  const globalRulesPath = path.join(os.homedir(), '.codex', 'rules', filename);

  if (fs.existsSync(localRulesPath)) return `${configDirName}/rules/${filename}`;
  if (fs.existsSync(globalRulesPath)) return `~/.codex/rules/${filename}`;

  // Backward compat: try workflows/ (legacy location)
  const localWorkflowsPath = path.join(baseDir, configDirName, 'workflows', filename);
  const globalWorkflowsPath = path.join(os.homedir(), '.codex', 'workflows', filename);

  if (fs.existsSync(localWorkflowsPath)) return `${configDirName}/workflows/${filename}`;
  if (fs.existsSync(globalWorkflowsPath)) return `~/.codex/workflows/${filename}`;
 ...
 * @returns {string|null} Resolved path or null
 */
function resolveScriptPath(filename, configDirName = '.codex', baseDir = process.cwd()) {
  const localPath = path.join(baseDir, configDirName, 'scripts', filename);
  const globalPath = path.join(os.homedir(), '.codex', 'scripts', filename);
  if (fs.existsSync(localPath)) return `${configDirName}/scripts/${filename}`;
  if (fs.existsSync(globalPath)) return `~/.codex/scripts/${filename}`;
  return null;
}
 ...
 */
function resolvePythonRunner(configDirName = '.codex', baseDir = process.cwd(), configuredRunner = null) {
  const isWindows = process.platform === 'win32';
  const venvBin = isWindows ? 'Scripts' : 'bin';
  const pythonNames = isWindows ? ['python.exe'] : ['python', 'python3'];
  const candidates = [];

  if (configuredRunner && configuredRunner !== 'auto') return configuredRunner;
  if (process.env.CK_PYTHON) candidates.push(process.env.CK_PYTHON);

  const homeVenv = path.join(os.homedir(), '.venv');
  for (const pythonName of pythonNames) {
    candidates.push(path.join(homeVenv, venvBin, pythonName));
    candidates.push(path.join(homeVenv, 'shared', venvBin, pythonName));
  }
  try {
    if (fs.existsSync(homeVenv)) {
      for (const entry of fs.readdirSync(homeVenv, { withFileTypes: true })) {
        if (!entry.isDirectory()) continue;
        for (const pythonName of pythonNames) {
          candidates.push(path.join(homeVenv, entry.name, venvBin, pythonName));
        }
      }
    }
  } catch {
    // Ignore unreadable ~/.venv layouts and continue with other candidates.
  }

  for (const pythonName of pythonNames) {
    candidates.push(path.join(baseDir, '.venv', venvBin, pythonName));
    candidates.push(path.join(baseDir, '.agents', 'skills', '.venv', venvBin, pythonName));
    candidates.push(path.join(baseDir, configDirName, 'skills', '.venv', venvBin, pythonName));
    candidates.push(path.join(os.homedir(), '.codex', 'skills', '.venv', venvBin, pythonName));
  }
...
function resolveSkillsVenv(configDirName = '.codex', baseDir = process.cwd(), configuredRunner = null) {
  return resolvePythonRunner(configDirName, baseDir, configuredRunner);
}
 ...
 */
function buildPlanContext(sessionId, config, baseDir = process.cwd()) {
  const { plan, paths } = config;
  const gitBranch = execSafe('git branch --show-current', baseDir);
  const resolved = resolvePlanPath(sessionId, config, { cwd: baseDir });
  const reportsPath = getReportsPath(resolved.path, resolved.resolvedBy, plan, paths, baseDir);

  // Compute naming pattern directly for reliable injection
  const namePattern = resolveNamingPattern(plan, gitBranch);

  const planLine = resolved.resolvedBy === 'session' || resolved.resolvedBy === 'project'
    ? `- Plan: ${resolved.path}`
...
function buildRulesSection({ devRulesPath, catalogScript, skillsVenv, plansPath, docsPath }) {
  ...
  // Issue #476: Use absolute paths to prevent LLM confusion in multi-AGENTS.md projects
  ...
  const pythonRunner = skillsVenv || (process.platform === 'win32' ? 'python' : 'python3');
  if (catalogScript) {
    lines.push(`- Activate skills: Run \`${pythonRunner} ${catalogScript} --skills\` to generate a skills catalog and analyze it, then activate the relevant skills that are needed for the task during the process.`);
  }
...
function buildNamingSection({ reportsPath, plansPath, namePattern }) {
  const reportsPrefix = reportsPath.endsWith(path.sep) || reportsPath.endsWith('/') ? reportsPath : `${reportsPath}/`;
  const plansPrefix = plansPath.endsWith(path.sep) || plansPath.endsWith('/') ? plansPath : `${plansPath}/`;
  return [
    `## Naming`,
    `- Report: \`${reportsPrefix}{type}-${namePattern}.md\``,
    `- Plan dir: \`${plansPrefix}${namePattern}/\``,
 ...
 * @param {string} [params.configDirName='.codex'] - Config directory name
 ...
 * }}
 */
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.codex', baseDir } = {}) {
  // Load config if not provided
  const effectiveBaseDir = baseDir || process.cwd();
  const cfg = config || loadConfig({ includeProject: false, includeAssertions: false, cwd: effectiveBaseDir });

  // Resolve paths
  const devRulesPath = resolveRulesPath('development-rules.md', configDirName, effectiveBaseDir);
  const catalogScript = resolveScriptPath('generate_catalogs.py', configDirName, effectiveBaseDir);
  const skillsVenv = resolveSkillsVenv(configDirName, effectiveBaseDir, cfg.python?.runner);

  // Build plan context
  const planCtx = buildPlanContext(sessionId, cfg, effectiveBaseDir);
  ...
  const params = {
    ...
    catalogScript,
    skillsVenv,
    reportsPath: effectiveBaseDir ? resolveConfiguredPath(effectiveBaseDir, planCtx.reportsPath, planCtx.reportsPath) : planCtx.reportsPath,
    plansPath: effectiveBaseDir ? resolveConfiguredPath(effectiveBaseDir, plansPathRel, 'plans') : plansPathRel,
    docsPath: effectiveBaseDir ? resolveConfiguredPath(effectiveBaseDir, docsPathRel, 'docs') : docsPathRel,
