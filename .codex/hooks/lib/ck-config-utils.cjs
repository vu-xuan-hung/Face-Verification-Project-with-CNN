/**
 * Shared utilities for CodexKit hooks
 *
...
const LOCAL_CONFIG_PATH = '.codex/.ck.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.codex', '.ck.json');
...
function readProjectActivePlanState(baseDir = process.cwd()) {
  const statePath = path.join(baseDir, '.codex', 'state', 'active-plan.json');
  try {
 ...
 * - 'session': Explicitly set via set-active-plan.cjs with CK_SESSION_ID → ACTIVE (directive)
 * - 'project': Explicit project-wide active plan via set-active-plan.cjs --project → ACTIVE (directive)
 ...
 * @param {Object} config - CodexKit config
 * @returns {{ path: string|null, resolvedBy: 'session'|'project'|'branch'|null }} Resolution result with tracking
 */
function resolvePlanPath(sessionId, config, options = {}) {
  const cwd = options.cwd || process.cwd();
  const plansDir = config?.paths?.plans || 'plans';
  const resolvedPlansDir = resolveConfiguredPath(cwd, plansDir, plansDir);
  ...
  for (const method of order) {
    switch (method) {
      ...
      case 'project': {
        const state = readProjectActivePlanState(cwd);
        if (state?.activePlan) {
          let resolvedPath = state.activePlan;
          if (!path.isAbsolute(resolvedPath) && state.sessionOrigin) {
            resolvedPath = path.join(state.sessionOrigin, resolvedPath);
          }
          return { path: resolvedPath, resolvedBy: 'project' };
        }
        break;
      }
      case 'branch': {
        try {
          ...
          if (slug && fs.existsSync(resolvedPlansDir)) {
            const entries = fs.readdirSync(resolvedPlansDir, { withFileTypes: true })
              .filter(e => e.isDirectory() && e.name.includes(slug));
            if (entries.length > 0) {
              return {
                path: path.join(resolvedPlansDir, entries[entries.length - 1].name),
                resolvedBy: 'branch'
              };
            }
          }
...
function resolveConfiguredPath(baseDir, pathValue, fallback) {
  const normalized = normalizePath(pathValue) || fallback;
  if (!baseDir) return normalized;
  return isAbsolutePath(normalized) ? normalized : path.join(baseDir, normalized);
}
 ...
 *   2. Global config (~/.codex/.ck.json) - user preferences
 *   3. Local config (./.codex/.ck.json) - project-specific overrides
 *
...
function loadConfig(options = {}) {
  ...
  const localConfig = loadConfigFromPath(path.join(projectRoot, LOCAL_CONFIG_PATH));
  ...
  try {
    ...
    result.python = merged.python || DEFAULT_CONFIG.python;
    // Hooks configuration
...
/**
 * Write environment variable to CODEX_ENV_FILE (with escaping)
 */
 ...
 * @param {string|null} resolvedBy - How plan was resolved ('session'|'project'|'branch'|null)
...
function getReportsPath(planPath, resolvedBy, planConfig, pathsConfig, baseDir = null) {
  ...
  const normalizedPlanPath = planPath && (resolvedBy === 'session' || resolvedBy === 'project') ? normalizePath(planPath) : null;
  ...
  // Return absolute path if baseDir provided. Absolute configured paths must stay absolute.
  if (baseDir) return resolveConfiguredPath(baseDir, reportPath, reportPath);
  return reportPath + '/';
}
...
/**
 * Extract task list ID from plan resolution for Codex Tasks coordination
 ...
 *
 * @param {{ path: string|null, resolvedBy: 'session'|'project'|'branch'|null }} resolved - Plan resolution result
 * @returns {string|null} Task list ID (plan directory name) or null
 */
function extractTaskListId(resolved) {
  if (!resolved || (resolved.resolvedBy !== 'session' && resolved.resolvedBy !== 'project') || !resolved.path) {
    return null;
  }
...
function isHookEnabled(hookName, options = {}) {
  const config = loadConfig({ includeProject: false, includeAssertions: false, includeLocale: false, cwd: options.cwd || process.cwd() });
