#!/usr/bin/env node
/**
 * PostToolUse hook: Tracks file modifications and reminds to run code_simplifier
 *
 * This hook fires after Edit/Write/MultiEdit operations and:
 * 1. Tracks modified files in the session
 * 2. After significant edits, injects a reminder to run code_simplifier
 *
 * Auto-trigger behavior:
 * - Tracks edit count per session
 * - After 5+ file edits, reminds about code_simplifier
...
function main() {
  try {
    ...
    // Check if we should remind about code_simplifier
    const shouldRemind =
    ...
    if (shouldRemind) {
      ...
      additionalContext = `\n\n[Code Simplification Reminder] You have modified ${session.modifiedFiles.length} files in this session. Consider using the \`code_simplifier\` agent to refine recent changes before proceeding to code review. This is a MANDATORY step in the workflow.`;
    }
