...
// Flags/predicates whose following value is a matcher/exclusion expression,
// not a path being read. Examples:
//   find . -name .git -prune
//   find . -path '*/node_modules/*' -prune
//   grep --exclude-dir node_modules ...
const VALUE_SKIP_FLAGS = [
  '--exclude', '--ignore', '--skip',
  '-x',             // tar exclude shorthand
  '-name', '-iname', // find name predicates
  '-path', '-ipath', // find path predicates
  '-regex', '-iregex',
...
const EXCLUDE_FLAGS = VALUE_SKIP_FLAGS;
...
function extractFromCommand(command) {
  ...
  const paths = [];
  const analysisCommand = stripValueSkipFlagArgs(command);

  // First, extract quoted strings (preserve spaces in paths)
  const quotedPattern = /["']([^"']+)["']/g;
  let match;
  while ((match = quotedPattern.exec(analysisCommand)) !== null) {
  ...
  const withoutQuotes = analysisCommand.replace(/["'][^"']*["']/g, ' ');
  ...
  for (const token of tokens) {
    ...
    if (isSkippableToken(token)) {
      if (VALUE_SKIP_FLAGS.includes(token)) {
        skipNextToken = true;
      }
      continue;
    }
...
/**
 * Remove values for matcher/exclusion flags before path extraction. This avoids
 * treating expressions such as `-name .git` or `--exclude-dir node_modules` as
 * direct access to blocked directories.
 *
 * @param {string} command - The command string
 * @returns {string}
 */
function stripValueSkipFlagArgs(command) {
  return command.replace(
    /(^|\s)(--exclude|--ignore|--skip|--exclude-dir|-x|-name|-iname|-path|-ipath|-regex|-iregex|-wholename|-iwholename|-fstype|-user|-group)\s+(?:"[^"]*"|'[^']*'|\S+)/g,
    '$1$2 '
  );
}
