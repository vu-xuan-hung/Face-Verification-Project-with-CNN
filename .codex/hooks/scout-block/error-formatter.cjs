 ...
 * @returns {string}
 */
function formatConfigPath(codexDir) {
  if (codexDir) {
    return path.join(codexDir, '.ckignore');
  }
  return '.codex/.ckignore';
}
 ...
 * @param {string} details.codexDir - Path to .codex directory
 * @returns {string}
 */
function formatBlockedError(details) {
  const { path: blockedPath, pattern, tool, codexDir } = details;
  const configPath = formatConfigPath(codexDir);
