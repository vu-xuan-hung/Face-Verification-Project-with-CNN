 ...
 *
 * Fires when Plan subagent completes. Reminds to invoke ck:cook before implementation.
...
try {
  ...
  try {
    ...
    console.log('Invoke ck:cook before implementing the plan');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`Best Practice: start a fresh execution context, then run ck:cook ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Best Practice: start a fresh execution context, then run ck:cook {full-absolute-path-to-plan.md}');
    }
