...
try {
  ...
  if (!isHookEnabled('session-init', { cwd: KIT_TARGET_ROOT })) {
    process.exit(0);
  }
...
function cleanupOrphanedShadowedSkills() {
  const projectCwd = process.env.CK_PROJECT_ROOT || process.cwd();
  const shadowedDir = path.join(projectCwd, '.agents', 'skills', '.shadowed');
  if (!fs.existsSync(shadowedDir)) return { restored: [], skipped: [], kept: [] };

  const skillsDir = path.join(projectCwd, '.agents', 'skills');
  const restored = [];
  const skipped = [];
 ...
 * Scans ~/.codex/teams/ for active team configs and checks membership.
 * Note: Returns first team found — Codex supports one team per session.
 * Note: Team lifecycle (creation/cleanup) is managed by Codex, not this hook.
 * @returns {{ teamName: string, memberCount: number } | null}
 */
function detectAgentTeam() {
  try {
    const teamsDir = path.join(os.homedir(), '.codex', 'teams');
...
async function main() {
  try {
    ...
    const effectiveCwd = data.cwd?.trim() || process.env.CK_PROJECT_ROOT || process.cwd();
    const kitTargetRoot = KIT_TARGET_ROOT;
    const envFile = process.env.CODEX_ENV_FILE;
    const source = data.source || 'unknown';
    const sessionId = data.session_id || null;

    const config = loadConfig({ cwd: kitTargetRoot });
    ...
    const resolved = resolvePlanPath(null, config, { cwd: kitTargetRoot });
    ...
    // Extract task list ID for Codex Tasks coordination (shared helper)
    ...
    // Resolve ckit artifacts against the selected kit target. effectiveCwd may
    // be a child repo, but plans/docs/reports must stay in the target root.
    ...
    if (envFile) {
      ...
      // Codex Tasks integration - enables multi-session/subagent coordination
      // Task list ID = plan directory name (shared across all sessions working on same plan)
      if (taskListId) {
        writeEnv(envFile, 'CODEX_TASK_LIST_ID', taskListId);
      }
      ...
      writeEnv(envFile, 'CK_WORKING_CWD', effectiveCwd);
      writeEnv(envFile, 'CK_REPORTS_PATH', resolveConfiguredPath(baseDir, reportsPath, 'plans/reports'));
      writeEnv(envFile, 'CK_DOCS_PATH', resolveConfiguredPath(baseDir, config.paths.docs, 'docs'));
      writeEnv(envFile, 'CK_PLANS_PATH', resolveConfiguredPath(baseDir, config.paths.plans, 'plans'));
      writeEnv(envFile, 'CK_PROJECT_ROOT', kitTargetRoot);
    ...
    if (hasCleanup) {
      ...
      if (shadowedCleanup.kept.length > 0) {
        ...
        console.log(`    Review .agents/skills/.shadowed/ and merge changes manually.`);
      }
    }
    ...
    if (teamInfo) {
      ...
      console.log(`    Team config: ~/.codex/teams/${teamInfo.teamName}/config.json`);
    ...
    if (staticEnv.gitRoot && staticEnv.gitRoot !== effectiveCwd) {
      console.log(`Subdirectory mode: Plans/docs follow the configured target paths`);
      console.log(`   Git root: ${staticEnv.gitRoot}`);
      console.log(`   Kit target: ${kitTargetRoot}`);
    }

    // MITIGATION: Issue #277 - Auto-compact can bypass interactive approval gates
    ...
    // Upstream bug: Codex CLI should preserve pending interactive state during compaction.
    if (source === 'compact') {
      console.log(`\nCONTEXT COMPACTED - APPROVAL STATE CHECK:`);
      console.log(`If you were waiting for user approval via an interactive prompt (e.g., Step 4 review gate),`);
      console.log(`you MUST re-confirm with the user before proceeding. Do NOT assume approval was given.`);
      console.log(`Ask the user to verify: "Context was compacted. Please confirm approval to continue."`);
    }
