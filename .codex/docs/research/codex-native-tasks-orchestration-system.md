# Codex Todos to Tasks

> **From Todos to Tasks: How Codex's Task System Enables Tasks-Based Collaboration**
>
> *By Rick Hightower | Jan 27, 2026 | Spillwave Solutions*
>
> Source: [pub.spillwave.com](https://pub.spillwave.com/codex-code-todos-to-tasks-5a1b0e351a1c)

---

Transform your development process with Codex's new Task system. Discover how to manage complex projects effortlessly while boosting productivity with session-scoped tasks. Then figure out how to persist tasks so they continue beyond the current session.

Codex's new Task system enhances AI agent workflows by transitioning from a "Todos" system to session-scoped tasks that manage complex dependencies and enable parallel execution. The hydration pattern allows tasks to be created from persistent specification files, bridging sessions and facilitating professional-grade development. Key features include task creation, updates, and retrieval, with a focus on managing dependencies and coordinating multiple agents effectively. This system is designed for multi-file projects, improving productivity through structured workflows. Then as a proof of concept we integrated Codex tasks with an Agentic Spec-Driven Development framework to demonstrate Task persistent for project continuity beyond a single session.

**Session-scoped orchestration meets persistent specifications for parallel AI agent workflows**

---

With the release of Codex 2.1.19, a fundamental shift occurred in how AI agents handle work. Simple tasks no longer need checklists. The model handles them implicitly, maintaining context and executing straightforward requests without external tracking. But for long-running, multi-file engineering projects, this implicit handling falls short.

The transition from Codex's previous "Todos" system to the native Task architecture addresses this gap. Tasks transform Codex from a reactive assistant into a sophisticated project orchestrator capable of managing complex dependency trees and coordinating parallel sub-agents.
...
## Understanding Codex tasks: The Four Tools

Codex provides four tools for task management, each serving a specific purpose in the orchestration workflow.
...
When you close your terminal or your session times out, your tasks are gone. The `~/.codex/tasks/` directory exists, but it only contains lock files for coordination, not task data. I verified this directly:
...
The hydration pattern solves the persistence problem by using external files as the source of truth and "hydrating" Codex tasks from them at session start.

### The Pattern

```
┌─────────────────────┐  Session Start  ┌──────────────────┐
│ .speckit/tasks.md   │ ──────────────► │  Codex tasks     │
...
The term "hydration" comes from frontend frameworks where server-rendered HTML is "hydrated" with JavaScript interactivity. Here, static markdown task lists are hydrated into live, trackable Codex tasks.
...
2. Create Codex tasks for each unchecked item
...
- **Destination:** Codex tasks (session-scoped) — Codex now has a temporary copy of the task list in its immediate working memory for the duration of this current interaction (session).

#### 2. The Work Phase

- **Action:** Work — This is the main interaction where you prompt Codex to perform the tasks loaded during the hydration phase (e.g., writing code, analyzing text, etc.).

#### 3. Session End: "Sync back"

- **Source:** Task Updates (completed) — As a result of the "Work" phase, Codex has generated new outputs, marked items as finished, or modified existing tasks.
- **Action:** "Sync back" — The changes made during the session are written from Codex's memory back to the permanent storage.
...
- **Persistence vs. Session Scope:** The crucial distinction is that the `.md` file lasts forever (persistent), while Codex's knowledge of those tasks only lasts for the current interaction (session-scoped).
...
When I asked Codex to hydrate these into native Tasks, here's what happened:
...
Codex automatically set up the dependency chain. The resulting task list shows blocked and ready tasks:
...
Here's the insight: **You're not asking Codex to figure out what to do. You're giving Codex a pre-computed execution plan.** It can improve or augment it, but the plan is there already.

The hydration pattern takes your persistent task structure — the plan and task hierarchy exists as markdown files, JIRA tickets, or database entries — and is loaded directly into Codex's session-scoped Task system. Codex now has:
...
You start work. Codex completes 12 of 28 tasks. You need to step away for lunch, or a meeting, or because it's 2 AM and you should sleep. No problem. The sync-back writes the current state to your persistent storage. When you return, hours or days later, you hydrate again, and Codex picks up exactly where you left off. The blueprint persists. Progress accumulates.

This is fundamentally different from traditional Codex interactions, where context evaporates between sessions. Your task structure survives. Your progress is durable. Your orchestration plan is reusable.
...
The killer feature of Codex tasks is parallel execution. You can spawn multiple sub-agents, each with scoped access to specific directories, working on independent features simultaneously.
...
The orchestrator (main Codex session) maintains the shared task list. When Agent A completes a task:
...
2. Let Codex work through them
3. Sync the results back

You're not just completing work. You're **teaching your specification what "done" looks like.**

Your `tasks.md` files become a living record of successful execution. Future Codex sessions don't just see what needs to be done — they see what was already accomplished and how it was accomplished. The git history shows the progression. The completed checkboxes show the path that worked.

This creates a **compound interest effect** for AI-assisted development:

- **Session 1:** Codex learns your project structure by executing the first 10 tasks
- **Session 2:** Codex sees Session 1's completed work, understands the patterns established, builds on them
- **Session 3:** Codex has the full context of Sessions 1 and 2, works faster and with fewer clarifications
...
Your project gains **institutional memory** that survives context windows, session timeouts, and even switching between different Codex instances. The spec files are the shared memory. Tasks are the execution engine. Together, they create a development system that gets smarter over time.
...
When your Rust backend and React frontend have zero file overlap, there's no reason to build them sequentially. Spawn two agents. Give each one its sandbox. Let them work simultaneously. And if there is overlap, this would be a good time to introduce git work-trees which Codex is a master of and you can even improve it with a little work-tree skills magic.
...
The beauty is in the coordination. Codex's Task system handles dependency resolution automatically. When Agent A completes a blocking task, Agent B sees it immediately and can proceed. No manual coordination required. No risk of conflicts when agents stay in their designated directories.
...
- **`tasks.md`:** Granular checklist that hydrates into Codex tasks
...
After the parallel agents completed their work on the `code_agent_context_hooks` project, I had Codex update the spec files. Here's what the sync-back looks like:
...
Next session, Codex reads these updated files and knows exactly where to resume.
...
1. **Ralph Wiggum Loop:** A community plugin using stop hooks to trap Codex in a loop until work completed. Named after the Simpsons character who famously said "I'm helping!" The plugin kept Codex focused but was fragile and single-minded.
...
3. **Native Tasks (~January 22, 2026):** Built into Codex 2.1+. First-class support for everything Ralphie pioneered, plus integration with the sub-agent spawning system.
...
### 2. Ask Codex to Hydrate Tasks

```
Read tasks.md and create Codex tasks for each unchecked item.
T2 and T3 should be blocked by T1.
```

### 3. Work Through the Tasks

Codex will create the tasks, work through them in order, and update status as it goes.
...
Codex tasks aren't a todo list replacement. They're a session-scoped orchestration system that enables professional AI-assisted development when combined with persistent specification files.
...
3. At session start, ask Codex to hydrate Tasks from your specs
...
## Appendix: The Prompt to Turn SDD into Codex tasks

### Start Prompt

```
Convert SDD opencode project to Codex tasks

I am currently using the /sdd skill with github speckit files stored under .speckit/.
I want to convert these into Codex task workflow, which has a similar structure
and stores tasks on the filesystem as a DAG.

Please migrate this project to use Codex tasks by:

  Reviewing the plans, tasks, features, and constitution under ./speckit
  Implementing them using Codex tasks
  Updating the ./speckit files accordingly
  Using the /sdd skill as needed to understand the current implementation

Then I want you to spin up subagents using skills you find under .codex/skills
...
**Rick Hightower** is a technology executive and data engineer who led ML/AI development at a Fortune 100 financial services company. He created **skilz**, the universal agent skill installer, supporting 30+ coding agents including Codex, Gemini, Copilot, and Cursor, and co-founded the world's largest agentic skill marketplace.
