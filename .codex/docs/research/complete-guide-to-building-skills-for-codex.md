# The Complete Guide to Building Skills for Codex

> Source: [OpenAI - The Complete Guide to Building Skills for Codex](https://resources.openai.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Codex.pdf)
...
A skill is a set of instructions — packaged as a simple folder — that teaches Codex how to handle specific tasks or workflows. Skills are one of the most powerful ways to customize Codex for your specific needs. Instead of re-explaining your preferences, processes, and domain expertise in every conversation, skills let you teach Codex once and benefit every time.

Skills are powerful when you have repeatable workflows: generating frontend designs from specs, conducting research with consistent methodology, creating documents that follow your team's style guide, or orchestrating multi-step processes. They work well with Codex's built-in capabilities like code execution and document creation. For those building MCP integrations, skills add another powerful layer helping turn raw tool access into reliable, optimized workflows.
...
- Developers who want Codex to follow specific workflows consistently
- Power users who want Codex to follow specific workflows
- Teams looking to standardize how Codex works across their organization
...
1. **First level (YAML frontmatter):** Always loaded in Codex's system prompt. Provides just enough information for Codex to know when each skill should be used without loading all of it into context.
2. **Second level (SKILL.md body):** Loaded when Codex thinks the skill is relevant to the current task. Contains the full instructions and guidance.
3. **Third level (Linked files):** Additional files bundled within the skill directory that Codex can choose to navigate and discover only as needed.

This progressive disclosure minimizes token usage while maintaining specialized expertise.

#### Composability

Codex can load multiple skills simultaneously. Your skill should work well alongside others, not assume it's the only capability available.

#### Portability

Skills work identically across Codex, Codex, and API. Create a skill once and it works across all surfaces without modification, provided the environment supports any dependencies the skill requires.
...
If you already have a working MCP server, you've done the hard part. Skills are the knowledge layer on top — capturing the workflows and best practices you already know, so Codex can apply them consistently.
...
|---|---|
| Connects Codex to your service (Notion, Asana, Linear, etc.) | Teaches Codex how to use your service effectively |
| Provides real-time data access and tool invocation | Captures workflows and best practices |
| What Codex *can* do | How Codex *should* do it |
...
- No external tools required — uses Codex's built-in capabilities
...
- **Users don't need to prompt Codex about next steps**
...
The YAML frontmatter is how Codex decides whether to load your skill. Get this right.
...
- XML angle brackets (`< >`)
- Skills with "codex" or "openai" in name (reserved)

**Why:** Frontmatter appears in Codex's system prompt. Malicious content could inject instructions.

### Writing Effective Skills

#### The Description Field

The description metadata provides just enough information for Codex to know when each skill should be used without loading all of it into context. This is the first level of progressive disclosure.
...
- **Manual testing in Codex** — Run queries directly and observe behavior. Fast iteration, no setup required.
- **Scripted testing in Codex** — Automate test cases for repeatable validation across changes.
...
> **Pro Tip:** Iterate on a single task before expanding. The most effective skill creators iterate on a single challenging task until Codex succeeds, then extract the winning approach into a skill. This leverages Codex's in-context learning and provides faster signal than broad testing. Once you have a working foundation, expand to multiple test cases for coverage.
...
The `skill-creator` skill — available in Codex via plugin directory or download for Codex — can help you build and iterate on skills. If you have an MCP server and know your top 2–3 workflows, you can build and test a functional skill in a single sitting — often in 15–30 minutes.
...
3. Upload to Codex via Settings > Capabilities > Skills
4. Or place in Codex skills directory
...
Skills have been published as an open standard. Like MCP, skills should be portable across tools and platforms — the same skill should work whether you're using Codex or other AI platforms. That said, some skills are designed to take full advantage of a specific platform's capabilities; authors can note this in the skill's compatibility field.
...
- Version control and management through the Codex Console
- Works with the Codex agent SDK for building custom agents

**When to use skills via the API vs. Codex:**

| Use Case | Best Surface |
|---|---|
| End users interacting with skills directly | Codex / Codex |
| Manual testing and iteration during development | Codex / Codex |
| Individual, ad-hoc workflows | Codex / Codex |
...
4. Test:
    - Ask Codex: "Set up a new project in [Your Service]"
```
...
> "Our MCP server gives Codex access to your Linear projects. Our skills teach Codex your team's sprint planning workflow. Together, they enable AI-powered project management."
...
- **Tool-first:** "I have Notion MCP connected" → Your skill teaches Codex the optimal workflows and best practices. Users have access; the skill provides expertise.
...
**Debugging approach:** Ask Codex: "When would you use the [skill name] skill?" Codex will quote the description back. Adjust based on what's missing.
...
1. **Verify MCP server is connected**
   - Codex: Settings > Extensions > [Your Service]
...
3. **Test MCP independently**
   - Ask Codex to call MCP directly (without skill)
...
**Symptom:** Skill loads but Codex doesn't follow instructions
...
- Skills Explained
- How to Create Skills for Codex
...
- GitHub: `openais/skills`
- Contains OpenAI-created skills you can customize

### Tools and Utilities

**skill-creator skill:**
- Built into Codex and available for Codex
...
- General questions: Community forums at the Codex developer community

**For Bug Reports:**
- GitHub Issues: `openais/skills/issues`
...
- Skills named with "codex" or "openai" prefix (reserved)
