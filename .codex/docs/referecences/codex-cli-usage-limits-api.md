# Codex CLI Usage Limits API - Technical Documentation

> Reusable implementation guide for integrating OpenAI's OAuth Usage API across platforms.

## Overview

This document details how to implement Codex CLI's Usage Limit API integration, including:
- Fetching usage quotas from OpenAI's OAuth API
...
### Endpoint
```
GET https://api.openai.com/api/oauth/usage
```
...
|----------|---------------|----------|
| macOS | Keychain | Service: `Codex-credentials` |
| Windows | File | `%USERPROFILE%\.codex\.credentials.json` |
| Linux | File | `~/.codex/.credentials.json` |
...
```typescript
async function getCredentialsFromMacKeychain(): Promise<CodexOAuthCredentials | null> {
  try {
    ...
    // Extract OAuth credentials from the codexAiOauth field
    if (parsed.codexAiOauth) {
      return parsed.codexAiOauth
    }
...
```typescript
async function getCredentialsFromFile(): Promise<CodexOAuthCredentials | null> {
  const credentialsPath = join(homedir(), '.codex', '.credentials.json')
...
```typescript
export function isCredentialsExpired(credentials: CodexOAuthCredentials): boolean {
...
```typescript
import { getCodexCredentials } from './codex-credentials-service'
...
export async function getUsageLimits(): Promise<UsageLimits | null> {
  ...
  const token = credentials?.accessToken || process.env.CODEX_OAUTH_TOKEN
  ...
  try {
    const response = await fetch('https://api.openai.com/api/oauth/usage', {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
...
router.get('/usage-limits', requireAuth, async (req, res) => {
  ...
  if (!limits) {
    return res.status(400).json({
      ...
      hint: 'Ensure Codex CLI is authenticated with `codex login` and token has proper scopes',
    })
  }
...
|----------|----------|-------------|
| `CODEX_OAUTH_TOKEN` | No | Override token (bypasses keychain/file) |
| `OPENAI_API_KEY` | No | API key for subprocess spawning |

**Token Priority Order:**
1. System credentials (Keychain on macOS, file on Windows/Linux)
2. `CODEX_OAUTH_TOKEN` environment variable
...
|-------|-------|----------|
| 401 Unauthorized | Invalid/expired token | Re-authenticate with `codex login` |
| 403 Forbidden | Token lacks scopes | Request proper OAuth scopes |
| No token available | Not authenticated | Run `codex login` first |
...
2. **Subprocess Isolation**: Strip sensitive env vars when spawning Codex CLI
