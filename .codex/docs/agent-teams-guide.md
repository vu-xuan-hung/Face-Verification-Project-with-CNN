...
> Feature added in CodexKit v2.11.x | Codex v2.1.33+ | Status: Experimental
> Source: https://github.com/openai/codex | Last synced: Feb 5, 2026
> Skill version: v2.1.0 — event-driven hooks, agent memory, Task restrictions

## Overview

Agent Teams enable multiple **independent Codex sessions** working in parallel, coordinated by a lead session. Unlike subagents (Task tool), teammates have their own context windows, communicate peer-to-peer, and share a task list.
...
Enable in `settings.json` (may be GA in Codex 2.1.33+):
```json
{
  "env": {
...
│  │  ~/.codex/teams/{name}/config.json   <- Team roster     │ │
│  │  ~/.codex/tasks/{name}/              <- Task list       | │
...
|-----------|----------|---------|
| Team config | `~/.codex/teams/{name}/config.json` | Team membership (name, agentId, agentType) |
| Task list | `~/.codex/tasks/{name}/` | Shared work items with dependencies |
| Lead | Main Codex session | Coordinates, assigns, synthesizes |
...
**Context inheritance:** Each teammate loads AGENTS.md, skills, agents, hooks, MCP servers — same as any fresh session. They do **NOT** inherit the lead's conversation history. Include task-specific context in the spawn prompt.
...
| Token cost | Lower (results summarized back) | Higher (N separate Codex instances) |
...
══════════════
  User prompt -> Lead calls Teammate(spawnTeam)
                 -> Team config at ~/.codex/teams/{name}/config.json
...
════════════════
  ...
  ~/.codex/teams/{name}/  -> deleted
  ~/.codex/tasks/{name}/  -> deleted
...
CLI override: `codex --teammate-mode in-process`
...
In v2.0.0, the lead polled `TaskList` every 30s to detect progress — wasting tokens and missing events between polls. v2.1.0 replaces polling with two Codex 2.1.33 hook events.
...
|-------|--------|---------|----------|
| `project` | code-reviewer, debugger, tester, planner | `.codex/agent-memory/` (gitignored) | Project-specific patterns, test frameworks, architecture |
| `user` | researcher | `~/.codex/agent-memory/` | Domain knowledge reusable across projects |
...
- **Version pairing:** Skill v2.1.0 requires Codex v2.1.33+ for hook events (TaskCompleted, TeammateIdle) and agent memory. Falls back to polling on older versions.
...
| File | Purpose |
|------|---------|
| `.codex/skills/team/SKILL.md` | Skill definition with imperative templates (v2.1.0) |
| `.codex/skills/team/references/` | Official docs reference (hooks, memory, restrictions) |
| `.codex/rules/team-coordination-rules.md` | Teammate behavior rules + CK conventions |
| `.codex/rules/orchestration-protocol.md` | Decision matrix, file ownership rules |
| `.codex/hooks/team-context-inject.cjs` | Team + CK context injection (SubagentStart) |
| `.codex/hooks/task-completed-handler.cjs` | Progress tracking + completion logging (TaskCompleted) |
| `.codex/hooks/teammate-idle-handler.cjs` | Task availability detection (TeammateIdle) |
| `.codex/hooks/session-init.cjs` | Team detection and env injection |

## Quick Start

1. Ensure `CODEX_EXPERIMENTAL_AGENT_TEAMS=1` is set
2. Tell Codex: `/team research "how should we optimize our database queries?"`
