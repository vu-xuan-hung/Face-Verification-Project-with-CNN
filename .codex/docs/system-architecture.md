...
CodexKit implements a multi-agent AI orchestration architecture where specialized agents collaborate through a file-based communication protocol. The system enables developers to leverage AI assistance throughout the entire software development lifecycle - from planning and implementation to testing, review, and deployment.
...
**Secondary Patterns**:
- Command Pattern (skill commands)
...
**Technology**: OpenAI Codex CLI / OpenCode AI CLI
...
mode: subagent | all
model: openai/codex-sonnet-4-20250514
temperature: 0.1
---
...
**Model Selection**:
- `codex-sonnet-4-20250514` - Fast, efficient (most agents)
- `codex-opus-4-1-20250805` - Advanced reasoning (planner-researcher)
...
**Purpose**: Intercept and control Codex operations for performance, context management, and security

**Hook Architecture**:
All hooks located in `.codex/hooks/` with consistent patterns - fail-safe exit code 0 (non-blocking)
...
- **Functionality**:
  - Injects current development rules from `.codex/rules/`
...
```json
{
  "hooks": {
    "SubagentStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "node ${CODEX_PROJECT_DIR}/.codex/hooks/subagent-init.cjs"
      }]
    }],
    "BeforeBash": [{
      "type": "command",
      "command": "node ${CODEX_PROJECT_DIR}/.codex/hooks/scout-block.js"
    }]
  }
}
```
...
**Architecture**:
```
.codex/skills/markdown-novel-viewer/
├── scripts/
...
1. Create agent definition file: `.codex/agents/my-agent.md`
...
1. Create skill directory: `.codex/skills/my-command/`
...
1. Create skill directory: `.codex/skills/my-skill/`
...
1. Define workflow in `.codex/rules/`
