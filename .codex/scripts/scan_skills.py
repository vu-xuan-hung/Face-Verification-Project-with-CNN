#!/usr/bin/env python3
"""
Scan .agents/skills directory and extract skill metadata.
"""
...
def scan_skills(base_path: Path) -> List[Dict]:
    ...
    for skill_file in sorted(base_path.rglob('SKILL.md')):
        ...
        try:
            ...
            skill_entry = {
                'name': skill_name,
                'path': str(skill_file.relative_to(Path('.agents/skills'))),
...
def main():
    ...
    legacy_path = Path('.codex/scripts/skills_data.yaml')
    runner = sys.executable or "python3"
    legacy_path.write_text(
        "# Skills catalog moved to .agents/skills/ck-help/scripts/skills_data.yaml\n"
        f"# Regenerate via: {runner} .codex/scripts/scan_skills.py\n",
        encoding='utf-8',
    )
