...
Outputs YAML to stdout by default for easy consumption by Codex.
...
try:
    ...
    raise SystemExit(
        f"PyYAML is required. Install with: {runner} -m pip install -r .codex/scripts/requirements.txt"
    )
...
def load_yaml(filename):
    ...
    # skills_data canonical source lives under .agents/skills/ck-help.
    if filename == 'skills_data.yaml':
        canonical = project_root / '.agents' / 'skills' / 'ck-help' / 'scripts' / 'skills_data.yaml'
        if canonical.exists():
        ...
        if installed_skills.exists():
            ...
            try:
                ...
                return scan_skills(Path('.agents/skills'))
            finally:
...
def generate_commands_yaml():
    ...
    catalog = {
        'metadata': {
            ...
            'description': 'Auto-generated catalog of all available commands in CodexKit',
...
def generate_skills_yaml():
    ...
    catalog = {
        'metadata': {
            ...
            'description': 'Auto-generated catalog of all available skills in CodexKit',
            'last_updated': datetime.now().strftime('%Y-%m-%d'),
