...
Centralized utility scripts for Codex skills.
...
Centralized environment variable resolver that follows Codex's hierarchy.

### Priority Order (Highest to Lowest)

1. **process.env** - Runtime environment variables (HIGHEST)
2. **PROJECT/.agents/skills/\<skill\>/.env** - Project skill-specific
3. **PROJECT/.agents/skills/.env** - Project shared across skills
4. **PROJECT/.codex/.env** - Project global defaults
5. **~/.codex/skills/\<skill\>/.env** - User skill-specific
6. **~/.codex/skills/.env** - User shared across skills
7. **~/.codex/.env** - User global defaults (LOWEST)

### CLI Usage

```bash
# Resolve a variable for a specific skill
python ~/.codex/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal

# With verbose output
python ~/.codex/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --verbose

# Find all locations where variable is defined
python ~/.codex/scripts/resolve_env.py GEMINI_API_KEY --find-all

# Show hierarchy for a skill
python ~/.codex/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal

# Export format for shell sourcing
eval $(python ~/.codex/scripts/resolve_env.py GEMINI_API_KEY --export)
```
...
if not api_key:
    ...
    print("Run: python ~/.codex/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
    sys.exit(1)
...
python ~/.codex/scripts/resolve_env.py TEST_VAR --verbose
...
python ~/.codex/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --find-all
```

## generate_catalogs.py

Generate YAML catalogs from command and skill data files. Outputs to stdout by default for easy consumption by Codex.

### Usage

```bash
# Generate skills catalog (outputs to stdout)
CK_PYTHON="${CK_PYTHON:-$HOME/.venv/shared/bin/python}"
"$CK_PYTHON" .codex/scripts/generate_catalogs.py --skills

# Generate skills catalog to file
"$CK_PYTHON" .codex/scripts/generate_catalogs.py --skills --output guide/SKILLS.yaml

# Write to file instead of stdout
"$CK_PYTHON" .codex/scripts/generate_catalogs.py --skills --output guide/SKILLS.yaml

# View help
"$CK_PYTHON" .codex/scripts/generate_catalogs.py --help
```
