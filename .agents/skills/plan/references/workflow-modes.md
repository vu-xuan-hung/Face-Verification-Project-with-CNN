...
Use `ask the user` if detection is uncertain.
...
All modes: See `task-management.md` for task creation patterns and metadata.
...
After creating: `node .codex/scripts/set-active-plan.cjs {plan-dir}`
