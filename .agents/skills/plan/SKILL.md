...
ALWAYS create plans or reports under the configured `Plan dir:` / `Reports:` paths from the injected `## Naming` and `## Paths` context.
For umbrella repositories, this may point at the umbrella root even when the current work context is a child repo.
...
| ``` validation workflow` | `references/validate-workflow.md` | Validate plan with critical questions interview |
