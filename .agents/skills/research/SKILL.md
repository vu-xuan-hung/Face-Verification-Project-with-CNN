...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.codex/.ck.json` (or `~/.codex/.ck.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use available web search.
   - **Gemini Model**: Read from `.codex/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
   ...
   - If `useGemini` is disabled or `gemini` bash command is not available, use `available web search` tool.
   - Run multiple `gemini` bash commands or `available web search` tools in parallel to search for relevant information.
