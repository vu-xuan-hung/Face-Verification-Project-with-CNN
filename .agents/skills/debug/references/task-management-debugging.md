...
Track investigation and debugging pipelines via runtime task tracking (task creation, task update, task list).
...
```
task creation: "Assess incident scope"      → pending
task creation: "Collect logs and evidence"  → pending, blockedBy: [assess]
task creation: "Analyze root cause"         → pending, blockedBy: [collect]
task creation: "Implement fix"              → pending, blockedBy: [analyze]
task creation: "Verify fix resolves issue"  → pending, blockedBy: [fix]
```
...
```
task creation(subject: "Re-analyze: fix attempt {N} failed",
...
If `task creation` fails: log warning, continue with sequential debugging. Tasks add visibility and coordination, not core functionality.
