...
Track investigation pipelines via runtime task tools when available. Hydration pattern for multi-step investigations with dependency chains and parallel evidence collection.
...
Visual verification of frontend implementations via Chrome MCP (Codex Chrome Extension) or `ck:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
