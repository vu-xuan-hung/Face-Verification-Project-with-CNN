...
This guide explains how to install dependencies for Codex skills.
...
```bash
cd .agents/skills/ai-multimodal/scripts
...
**ai-multimodal** (`.agents/skills/ai-multimodal/scripts/requirements.txt`):
...
# Python packages (ai-multimodal only)
cd .codex\skills\ai-multimodal\scripts
pip install -r requirements.txt
...
```bash
cd .agents/skills/{skill-name}/scripts
...
2. **`.agents/skills/{skill-name}/.env`** (skill-specific config)
3. **`.agents/skills/.env`** (shared skills config)
4. **`.codex/.env`** (global Codex config)
...
# Run all tests
pytest .agents/skills/*/scripts/tests/ -v

# Check coverage across all skills
pytest .agents/skills/*/scripts/tests/ --cov=.agents/skills --cov-report=html
```
...
5. Open an issue: https://github.com/openais/codex-code/issues
