...
# Installs all dependencies for Codex skills
#
...
setup_python_env() {
    ...
    # Install .codex/scripts requirements (contains pyyaml for generate_catalogs.py)
    local SCRIPTS_REQ="$SCRIPT_DIR/../scripts/requirements.txt"
    if [ -f "$SCRIPTS_REQ" ]; then
        local SCRIPTS_LOG="$LOG_DIR/install-scripts.log"
        print_info "Installing .codex/scripts dependencies..."
        ...
        if [[ $pkg_fail -eq 0 ]]; then
            print_success ".codex/scripts: all $pkg_success packages installed"
            track_success "optional" "scripts"
        else
            print_warning ".codex/scripts: $pkg_success installed, $pkg_fail failed"
        fi
    fi
...
print_usage() {
    ...
    echo -e "  source .agents/skills/.venv/bin/activate"
    echo ""
    echo -e "${BLUE}For more information, see:${NC}"
    echo -e "  .agents/skills/INSTALLATION.md"
    echo ""
}
...
main() {
    ...
    print_header "Codex Skills Installation"
    print_info "OS: $OS"
    if [[ "$OS" == "linux" ]] || [[ "$OS" == "wsl" ]]; then
