...
function Setup-PythonEnv {
    ...
    if (Test-Path $scriptsReqPath) {
        ...
        Write-Info "Installing .codex/scripts dependencies..."
        ...
        if ($pkgFail -eq 0) {
            Write-Success ".codex/scripts: all $pkgSuccess packages installed"
            Track-Success -Category "optional" -Name "scripts"
        } else {
            Write-Warning ".codex/scripts: $pkgSuccess installed, $pkgFail failed"
        }
    }
