...
1. Scan the codebase and calculate the number of files with LOC in each directory (skip `.codex`, `.opencode`, `.git`, `tests`, `node_modules`, `__pycache__`, `secrets`, etc.)
...
3. Strategy:
   - 1-3 files: Skip parallel reading, docs_manager reads directly
...
6. Merge results into context for docs_manager

## Phase 2: Documentation Update (docs_manager Agent)

**CRITICAL:** You MUST spawn `docs_manager` agent via Task tool with merged reports and doc readings.

Pass the gathered context to docs_manager agent to update documentation:
...
1. Run: `node .codex/scripts/validate-docs.cjs docs/`
