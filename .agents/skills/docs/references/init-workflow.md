...
1. Scan the codebase and calculate the number of files with LOC in each directory (skip credentials, cache or external modules directories, such as `.codex`, `.opencode`, `.git`, `tests`, `node_modules`, `__pycache__`, `secrets`, etc.)
...
## Phase 2: Documentation Creation (docs_manager Agent)

**CRITICAL:** You MUST spawn `docs_manager` agent via Task tool with merged reports. Do not wait for user input.

Pass the gathered context to docs_manager agent to create initial documentation:
...
3. For files exceeding limit:
   ...
   - docs_manager should have already split proactively
