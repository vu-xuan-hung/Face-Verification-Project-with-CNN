...
Track review pipeline execution via runtime task tracking (tracked task creation, tracked task update, tracked task list).
...
tracked task creation(subject: "Review backend auth changes",
  metadata: { reviewStage: "review", scope: "src/api/,src/middleware/",
              agentIndex: 1, totalAgents: 2, priority: "P1" })

tracked task creation(subject: "Review frontend auth UI",
  metadata: { reviewStage: "review", scope: "src/components/auth/",
              agentIndex: 2, totalAgents: 2, priority: "P1" })

// Both run simultaneously via separate code_reviewer subagents
// Fix task blocks on BOTH completing:
tracked task creation(subject: "Fix all review issues",
...
```
tracked task creation(subject: "Re-review after fixes",
  addBlockedBy: ["{fix-task-id}"],
...
If `tracked task creation` fails: log warning, fall back to sequential review without task tracking. Review pipeline functions identically — tasks add visibility, not functionality.
