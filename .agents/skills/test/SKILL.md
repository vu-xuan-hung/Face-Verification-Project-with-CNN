...
If invoked with context (test scope), proceed with testing. If invoked WITHOUT arguments, ask the user to choose available test operations:
...
Present options with header "Test Operation", question "What would you like to do?".
...
1. On start: check `tracked task list`, claim assigned/next unblocked task via `tracked task update`
...
5. When done: `tracked task update(status: "completed")` then `SendMessage` results to lead
