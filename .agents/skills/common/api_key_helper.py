...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    codex_env = project_dir / '.codex' / '.env'
    if codex_env.exists():
        api_key = load_env_file(codex_env)
        if api_key:
            print(f"✓ Using API key from {codex_env}", file=sys.stderr)
            return api_key

    # Step 4: Check ./.agents/skills/.env
    codex_skills_env = project_dir / '.agents' / 'skills' / '.env'
    if codex_skills_env.exists():
        api_key = load_env_file(codex_skills_env)
        if api_key:
            print(f"✓ Using API key from {codex_skills_env}", file=sys.stderr)
            return api_key
...
def get_api_key_or_exit(skill_dir: Optional[Path] = None) -> str:
    ...
    if not api_key:
        ...
        print("\n3️⃣  .codex/.env file:", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.codex/.env", file=sys.stderr)

        print("\n4️⃣  .agents/skills/.env file (shared across all Gemini skills):", file=sys.stderr)
        print(f"   echo 'GEMINI_API_KEY=your-api-key' > {project_dir}/.agents/skills/.env", file=sys.stderr)
