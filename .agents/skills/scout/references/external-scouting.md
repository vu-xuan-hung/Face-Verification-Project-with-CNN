...
Use `Task` tool with `agent type: "Bash"` to spawn parallel agents:

```
Task 1: agent type="Bash", prompt="Run: gemini -y -m gemini-3-flash-preview '[prompt1]'"
Task 2: agent type="Bash", prompt="Run: gemini -y -m gemini-3-flash-preview '[prompt2]'"
Task 3: agent type="Bash", prompt="Run: gemini -y -m gemini-3-flash-preview '[prompt3]'"
```
...
```
Task 1: agent type="Bash", prompt="cat file1.ts file2.ts"
Task 2: agent type="Bash", prompt="cat file3.ts file4.ts"
```

**Large file (>500 lines) - use sed for ranges:**
```
Task 1: agent type="Bash", prompt="sed -n '1,500p' large-file.ts"
Task 2: agent type="Bash", prompt="sed -n '501,1000p' large-file.ts"
Task 3: agent type="Bash", prompt="sed -n '1001,1500p' large-file.ts"
```
