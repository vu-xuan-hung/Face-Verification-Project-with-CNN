...
3. Spawn parallel agents with divided directories when available, or run parallel file reads/searches
...
- If task tools exist, check existing scout tasks first
- If not found, create one tracked task per agent with scope metadata
...
**Notes:**
- Mark each tracked task `in_progress` before spawning its agent when task tools exist
...
- Mark completed tracked tasks; log timed-out agents in report
...
- `references/task-management-scouting.md` - Task-tracking patterns for scout coordination
