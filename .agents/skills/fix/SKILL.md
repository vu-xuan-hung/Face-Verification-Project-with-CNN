...
- `--parallel` - Activate parallel mode: route to parallel `fullstack_developer` agents per issue
...
See `references/mode-selection.md` for the mode-selection prompt format.

### Step 2: Debug

- Activate `ck:debug` skill.
- Guess all possible root causes.
- Prefer multiple `Explore`/scout agents in parallel to verify each hypothesis when the runtime exposes delegation; otherwise use parallel searches and report the limitation.
...
| **Parallel** | 2+ independent issues OR `--parallel` flag | Parallel `fullstack_developer` agents |

**Task Orchestration (Moderate+ only):** After classifying, use available task-tracking tools for all phases upfront with dependencies. See `references/task-orchestration.md`.
- Skip for Quick workflow (< 3 steps, overhead exceeds benefit)
- Create tracked tasks with dependency chains when task tools exist
- Update task/checklist state as each phase completes
...
- Prefer multiple scout agents to verify no regressions when available.
...
3. Update task state/checklists and mark all tracked tasks completed
4. Ask user if they want to commit via `git_manager` subagent
...
**Subagents:** `debugger`, `researcher`, `planner`, `code_reviewer`, `tester`, `Bash`
...
Load as needed:
- `references/mode-selection.md` - Prompt format for mode
- `references/complexity-assessment.md` - Classification criteria
- `references/task-orchestration.md` - Native Codex Task patterns for moderate+ workflows
