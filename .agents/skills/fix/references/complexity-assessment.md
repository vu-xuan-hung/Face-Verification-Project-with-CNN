...
### Parallel (→ multiple fullstack_developer agents) — Use Task Trees
...
**Task usage:** Create separate task trees per independent issue. Spawn `fullstack_developer` agent per tree. Agents coordinate via `task update`/`task list`. See `references/task-orchestration.md`.
